  ...
  <v-dialog
    ...
    <template v-slot:activator="{ on, attrs }">
      <v-btn
        ...
        depressed
        small>
        <v-icon class="mr-1">mdi-pencil</v-icon> Edit Rate
      </v-btn>
    </template>
    <v-card>
      <v-row no-gutters>
        ...
        <v-col cols="12" class="pa-2 pt-3">
          <DeleteMaterialDialog :item="item" v-on:deleteItem="onDeleteItem" />
          <v-btn
            text
            color="success"
            class="float-right"
...
import DeleteMaterialDialog from '../delete/DeleteMaterialDialog.vue';
...
export default class EditSandGravelMaterial extends Vue {
  @Prop({ required: true }) public readonly item!: _object;
  public dialogState = false;

  @Watch('item.material')
  onUpdateName() {
    this.item.type = this.item.material.toLowerCase().replaceAll(' ', '_');
  }
