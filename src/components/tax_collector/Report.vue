  ...
  <v-container class="pa-0">
    <v-row class="px-4 pb-3">
      <v-col class="px-0 pb-0" cols="7">
        <h1>Collector Reports</h1>
      </v-col>
      <!-- <v-col class="px-0 pb-0 d-flex justify-end align-center" cols="5">
        <v-btn color="success" class="white--text" @click="sortModal = true">
          Generate
          <v-icon right dark>
            mdi-file-document
          </v-icon>
...
import LiquidatorReport from "./components/reports/LiquidatorReport.vue";
...
export default class TaxCollectorReports extends Vue {
  ...
  private selectReports() {
    this.collector_reports = this.reports.filter(
      (r) => r.created_at == this.report_date
    );
  }

  private generatePdf() {
    this.selectReports();
    if (this.collector_reports.length > 0) {
      this.showPdfGen = true;
    } else {
      this.$store.commit("snackBar/toggle", {
        message: `No report for ${this.report_date}`,
        color: "error",
        toggle: true,
      });
    }
