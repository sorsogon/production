  ...
  >
    <v-sheet height="100%" class="grey darken-4 d-flex justify-center">
      <v-toolbar absolute width="100%" color="grey darken-4" dense>
        <h4 class="white--text">{{ `Liquidation_Report(${this.lr_date})` }}</h4>
...
export default {
  ...
  computed: {
    total: function() {
      return this.collections.reduce((total, curr) => {
    ...
    control_number: function() {
      ...
      return `${m}${day}${year}-${hour}${min}${sec}-LO${this.$root.$user.id}`;
    },
    lr_date: function() {
      const r_month = new Date(this.r_date).getMonth();
      const r_year = new Date(this.r_date).getFullYear();
      const r_day = new Date(this.r_date).getDate();

      let m = this.months[r_month];
      let report_date = `${m} ${r_day},${r_year}`;
  ...
  methods: {
    ...
    generatePdf: function() {
      const document = {
        info: {
          title: `Collection Report ${this.r_date}`,
        },
                ...
                columns: [
                  {
                    ...
                    stack: [
                      ...
                      {
                        text: `Cashier ${this.$root.$user.id} - Designee`,
                        alignment: "center",
                        fontSize: 10,
                      },
                    ],
      ...
      this.collections.forEach((collection) => {
        document.content[1].table?.body.push([
          { text: collection.serial_number, alignment: "right" },
          { text: collection.collection_officer },
          { text: collection.desc, alignment: "center" },
          { text: this.formatter(collection.amount), alignment: "right" },
        ]);
      });
    ...
    printPdf: function() {
      const l_report = this.$refs.liquidatorReport;
      let doc = l_report.contentWindow;
      if (doc != null) {
        doc.focus();
        doc.print();
      }
    ...
    uploadPdf: async function() {
      ...
      const report_data = {
        ...
        filename: `Liquidation_Report(${this.lr_date})`,
        type: "liquidator",
        desc: `Collection`,
        note: "none",
        document: this.data_uri,
        serial_number: `${m}${day}${year}-LO${this.$root.$user.id}`,
        amount: this.total,
